...
function sepiaFW_build_frames(){
	...
	Frames.open = function(info){
		...
		if (isThisFrameActive){
			...
			activitySettings = mergeObjectWithScopeEntry(info.activitySettings, "activitySettings", info.autoFillFrameEvents);
		}else{
			Frames.currentScope = {};
		}
		var $framesView = $('#sepiaFW-frames-view');
		var $framePages = $('.sepiaFW-frames-page');
		...
		$framesView.removeClass('dark');
		$framePages.removeClass('dark');
		$framePages.removeClass('flat');
		...
		if (info.theme){
			...
			if (info.theme.indexOf("dark") >= 0){
				$framesView.addClass('dark');
				$framePages.addClass('dark');
			}
			/*
		...
		if (!isThisFrameActive){
			...
			//OPEN frame
			$framesView.slideDown(300, function(){
				Frames.uic.refresh();
			});
			Frames.isOpen = true;
		...
		applyOrResetActivitySettings(activitySettings, false);
		
		//on open
		if(onOpen) onOpen(info.data, $framesView.get(0), Frames.currentScope);	//data, view, scope
	...
	Frames.close = function(){
		...
		//activity reset
		applyOrResetActivitySettings(activitySettingsBackup, true);
		//close
		var $framesView = $('#sepiaFW-frames-view');
	...
	Frames.handleMessages = function(msgObject){
		if (onMessageHandler) onMessageHandler(msgObject, Frames.currentScope);		//msg, scope
	}
	...
	Frames.handleMissedMessages = function(msgObject){
		if (onMissedMessageHandler) onMissedMessageHandler(msgObject, Frames.currentScope);		//msg, scope
	}
	...
	Frames.handleSpeechToTextInput = function(msgObject){
		if (onSpeechToTextInputHandler){
			return onSpeechToTextInputHandler(msgObject, Frames.currentScope);		//msg, scope
		}
	}
	...
	Frames.handleChatOutput = function(msgObject){
		if (onChatOutputHandler){
			onChatOutputHandler(msgObject, Frames.currentScope);		//msg, scope
		}
	}
	...
	function mergeObjectWithScopeEntry(obj, objName, fillViaScope){
		if (!obj) obj = {};
		if (fillViaScope){
			var scopeObj = Frames.currentScope[objName];
			if (!scopeObj){
				scopeObj = {};
				Frames.currentScope[objName] = scopeObj;	//NOTE: updates scope
			}
			return $.extend(true, scopeObj, obj);
		}else{
			return obj;
		}
	}

	function applyOrResetActivitySettings(newSettings, isReset){
		if (!isReset) activitySettingsBackup = {};
		if (newSettings.language){
			if (!isReset) activitySettingsBackup.language = SepiaFW.config.appRegionCode || SepiaFW.config.appLanguage;
			SepiaFW.ui.actions.switchLanguage({language_code: newSettings.language, skip_save: true});
		}
		if (newSettings.defaultTask){
			if (!isReset) activitySettingsBackup.defaultTask = SepiaFW.assistant.getDefaultTask();
			SepiaFW.assistant.setDefaultTask(newSettings.defaultTask);
		}
		//TODO: add more, e.g.: asrModel, ttsVoice
	}
